...
public class ConfigurationPanel extends JPanel {
    ...
    private void refreshUI() {
        SwingUtilities.invokeLater(() -> {
            ...
            boolean currentAuthEnabled = authEnabledCheckbox.isSelected();
            String currentApiToken = new String(apiTokenField.getPassword());
            ...
            authEnabledCheckbox.setSelected(currentAuthEnabled);
    ...
    private void initializeUI() {
        ...
        contentPanel.add(createApiAuthConfigSection());
        contentPanel.add(Box.createVerticalStrut(20));
    ...
    private JPanel createServerConfigSection() {
        ...
        JLabel portHelpLabel = new JLabel("Restart extension to apply port change");
    ...
    /**
     * Creates the API Authentication configuration section.
     */
    private JPanel createApiAuthConfigSection() {
        JPanel section = createStyledSection("API Authentication");
        GridBagConstraints gbc = new GridBagConstraints();
        ...
        gbc.weightx = 1.0;
        gbc.gridwidth = 2;
        authEnabledCheckbox = new JCheckBox("Require API token for requests");
        authEnabledCheckbox.setToolTipText("When enabled, all endpoints (except /health, /docs, /openapi, /postman, /version) require X-Belch-Token or Authorization: Bearer <token>");
        authEnabledCheckbox.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 12));
        authEnabledCheckbox.setBackground(sectionBackground);
        authEnabledCheckbox.setForeground(textColor);
        authEnabledCheckbox.setOpaque(true);
        section.add(authEnabledCheckbox, gbc);

        // API Token label
        gbc.gridx = 0; gbc.gridy = 1;
        gbc.gridwidth = 1;
        gbc.insets = new Insets(12, 5, 5, 10);
        JLabel tokenLabel = new JLabel("API Token:");
        tokenLabel.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 12));
        tokenLabel.setForeground(textColor);
        section.add(tokenLabel, gbc);

        // Token field + auto-generate button in one row
        gbc.gridx = 1;
        gbc.insets = new Insets(12, 0, 5, 5);
        gbc.weightx = 1.0;
        gbc.fill = GridBagConstraints.HORIZONTAL;

        JPanel tokenFieldPanel = new JPanel(new BorderLayout(5, 0));
        tokenFieldPanel.setBackground(sectionBackground);
        tokenFieldPanel.setOpaque(true);

        apiTokenField = new JPasswordField(28);
        apiTokenField.setToolTipText("Secret token; send as header X-Belch-Token or Authorization: Bearer <token>.");
        apiTokenField.setPreferredSize(new Dimension(280, 28));
        apiTokenField.setBorder(BorderFactory.createCompoundBorder(
            BorderFactory.createLineBorder(borderColor, 1),
            BorderFactory.createEmptyBorder(4, 8, 4, 8)
        ));
        apiTokenField.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 12));
        apiTokenField.setBackground(sectionBackground);
        apiTokenField.setForeground(textColor);
        apiTokenField.setCaretColor(textColor);
        apiTokenField.setEchoChar('*');
        apiTokenField.setOpaque(true);
        tokenFieldPanel.add(apiTokenField, BorderLayout.CENTER);

        JPanel tokenButtonsPanel = new JPanel(new FlowLayout(FlowLayout.RIGHT, 5, 0));
        tokenButtonsPanel.setBackground(sectionBackground);
        tokenButtonsPanel.setOpaque(true);

        JButton showHideButton = new JButton("Show");
        showHideButton.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 11));
        showHideButton.setBackground(borderColor);
        showHideButton.setForeground(textColor);
        showHideButton.setBorder(BorderFactory.createEmptyBorder(6, 10, 6, 10));
        showHideButton.setFocusPainted(false);
        showHideButton.setToolTipText("Show token to copy");
        showHideButton.addActionListener(e -> {
            if (apiTokenField.getEchoChar() == '*') {
                apiTokenField.setEchoChar((char) 0);
                showHideButton.setText("Hide");
                showHideButton.setToolTipText("Hide token");
            } else {
                apiTokenField.setEchoChar('*');
        ...
        tokenButtonsPanel.add(showHideButton);

        JButton copyTokenButton = new JButton("Copy");
        copyTokenButton.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 11));
        copyTokenButton.setBackground(borderColor);
        copyTokenButton.setForeground(textColor);
        copyTokenButton.setBorder(BorderFactory.createEmptyBorder(6, 10, 6, 10));
        copyTokenButton.setFocusPainted(false);
        copyTokenButton.setToolTipText("Copy token to clipboard");
        copyTokenButton.addActionListener(e -> {
            String token = new String(apiTokenField.getPassword());
            if (token == null || token.isEmpty()) {
                updateStatus("No API token to copy.", WARNING_COLOR);
                return;
            }
            Clipboard clipboard = Toolkit.getDefaultToolkit().getSystemClipboard();
            clipboard.setContents(new StringSelection(token), null);
            updateStatus("API token copied to clipboard.", SUCCESS_COLOR);
        });
        tokenButtonsPanel.add(copyTokenButton);

        JButton generateTokenButton = createStyledButton("Auto-Generate", SUCCESS_COLOR);
        generateTokenButton.setToolTipText("Generate a strong random API token");
        generateTokenButton.addActionListener(e -> {
            String token = generateRandomToken();
            apiTokenField.setText(token);
            updateStatus("Generated API token; click Save Configuration to apply.", SUCCESS_COLOR);
        });
        tokenButtonsPanel.add(generateTokenButton);

        tokenFieldPanel.add(tokenButtonsPanel, BorderLayout.EAST);

        section.add(tokenFieldPanel, gbc);

        gbc.gridx = 0; gbc.gridy = 2;
        gbc.gridwidth = 2;
        gbc.insets = new Insets(2, 5, 5, 5);
        gbc.fill = GridBagConstraints.NONE;
        JLabel authHelpLabel = new JLabel("Click Show to reveal and copy the token. Token applies immediately after Save.");
        authHelpLabel.setFont(new Font(Font.SANS_SERIF, Font.ITALIC, 11));
        authHelpLabel.setForeground(textMuted);
        section.add(authHelpLabel, gbc);
    ...
    private JPanel createInfoPanel() {
        ...
        infoText.setText(
            ...
            "API token and authentication toggle apply immediately after Save. Port and database path require extension reload.\n" +
    ...
    private void loadCurrentSettings() {
        ...
        if (savedProps != null) {
            ...
            authEnabledCheckbox.setSelected(Boolean.parseBoolean(savedProps.getProperty("api.auth.enabled", String.valueOf(config.isAuthEnabled()))));
            apiTokenField.setText(savedProps.getProperty("api.auth.token", config.getApiToken() != null ? config.getApiToken() : ""));
        } else {
            ...
            authEnabledCheckbox.setSelected(config.isAuthEnabled());
            apiTokenField.setText(config.getApiToken() != null ? config.getApiToken() : "");
        }
    ...
    private void saveConfiguration() {
        try {
            ...
            props.setProperty("api.auth.enabled", String.valueOf(authEnabledCheckbox.isSelected()));
            props.setProperty("api.auth.token", new String(apiTokenField.getPassword()).trim());
            ...
            config.setAuthEnabled(authEnabledCheckbox.isSelected());
            config.setApiToken(new String(apiTokenField.getPassword()).trim());
            ...
            JOptionPane.showMessageDialog(this,
                ...
                "API token and auth enable/disable apply immediately.\n" +
                "Port and database path require extension reload to take effect.",
                "Configuration Saved",
    ...
    /**
     * Generates a cryptographically strong random token for API authentication.
     */
    ...
    public static void loadSavedConfigInto(ApiConfig config) {
        ...
        try {
            ...
            if (props.containsKey("api.auth.enabled")) {
                config.setAuthEnabled(Boolean.parseBoolean(props.getProperty("api.auth.enabled")));
            }
            if (props.containsKey("api.auth.token")) {
                config.setApiToken(props.getProperty("api.auth.token"));
            }
