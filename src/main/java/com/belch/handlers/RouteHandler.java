...
public class RouteHandler {
    ...
    // Custom exception used to short-circuit request processing on auth failure
    private static class ApiAuthException extends RuntimeException {
        private final int status;
        private final Map<String, Object> body;

        ApiAuthException(int status, Map<String, Object> body) {
            super((String) body.getOrDefault("message", "API authentication error"));
            this.status = status;
            this.body = body;
        }
    ...
    public void registerRoutes(Javalin app) {
        ...
        // Add simple token-based API authentication (optional)
        app.before(ctx -> {
            ...
            String expectedToken = config.getApiToken();
            if (expectedToken == null || expectedToken.isEmpty()) {
                logger.warn("API authentication is enabled but no api.auth.token is configured");
                throw new ApiAuthException(500, Map.of(
                    "error", "Server configuration error",
                    "message", "API authentication is enabled but no API token is configured"
                ));
            }

            // Prefer custom header, fallback to Bearer token
            String providedToken = ctx.header("X-Belch-Token");
            ...
            if (providedToken == null || !expectedToken.equals(providedToken)) {
                throw new ApiAuthException(401, Map.of(
                    "error", "Unauthorized",
        ...
        app.exception(Exception.class, (exception, ctx) -> {
            if (exception instanceof ApiAuthException authEx) {
                Map<String, Object> body = authEx.getBody();
                Map<String, Object> errorResponse = new HashMap<>(body);
                errorResponse.putIfAbsent("timestamp", System.currentTimeMillis());
                errorResponse.putIfAbsent("path", ctx.path());
                ctx.status(authEx.getStatus()).json(errorResponse);
                return;
            }
