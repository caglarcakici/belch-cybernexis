...
public class ApiConfig {
    ...
    private static final String AUTH_ENABLED_KEY = "api.auth.enabled";
    private static final String AUTH_TOKEN_KEY = "api.auth.token";
    ...
    public ApiConfig() {
        ...
        // Authentication configuration (API-level); UI-saved values used as fallback
        this.authEnabled = getBooleanProperty(properties, AUTH_ENABLED_KEY, "BURP_API_AUTH_ENABLED", this.authEnabled);
        this.apiToken = getStringProperty(properties, AUTH_TOKEN_KEY, "BURP_API_AUTH_TOKEN", this.apiToken != null ? this.apiToken : DEFAULT_API_TOKEN);

        // UI-saved config (extension.properties) overrides classpath application.properties so port/auth etc. persist after reload
        reApplyUiConfigOverride();

        logger.info("Configuration loaded - Port: {}, Database: {}, Verbose: {}, Session Tag: '{}', Auth Enabled: {}",
                   port, databasePath, verboseLogging, sessionTag, authEnabled);
    ...
    /**
     * Re-applies config from extension.properties (UI-saved) so it overrides application.properties.
     * Ensures port, auth, and other UI settings persist after extension reload.
     */
    private void reApplyUiConfigOverride() {
        String configDir = System.getProperty("user.home") + File.separator + ".belch";
        File uiConfigFile = new File(configDir + File.separator + "extension.properties");
        ...
        try {
            ...
            if (uiProps.containsKey(PORT_KEY)) {
                this.port = Integer.parseInt(uiProps.getProperty(PORT_KEY).trim());
                logger.info("UI config override: port={}", this.port);
            }
            if (uiProps.containsKey(DB_PATH_KEY)) {
                String configuredDbPath = uiProps.getProperty(DB_PATH_KEY).trim();
                if (!configuredDbPath.startsWith("/") && !configuredDbPath.contains(":")) {
                    String defaultDbDir = System.getProperty("user.home") + File.separator + ".belch";
                    this.databasePath = defaultDbDir + File.separator + configuredDbPath;
                } else {
            ...
            if (uiProps.containsKey(SESSION_TAG_KEY)) {
                this.sessionTag = generateSessionTagIfNeeded(uiProps.getProperty(SESSION_TAG_KEY));
            }
            if (uiProps.containsKey(VERBOSE_LOGGING_KEY)) {
                this.verboseLogging = Boolean.parseBoolean(uiProps.getProperty(VERBOSE_LOGGING_KEY));
            }
            if (uiProps.containsKey(AUTH_ENABLED_KEY)) {
                this.authEnabled = Boolean.parseBoolean(uiProps.getProperty(AUTH_ENABLED_KEY));
            }
            if (uiProps.containsKey(AUTH_TOKEN_KEY)) {
                this.apiToken = uiProps.getProperty(AUTH_TOKEN_KEY);
            }
        } catch (Exception e) {
            logger.debug("Could not re-apply UI config override: {}", e.getMessage());
        }
    }
    ...
    private void loadFromUiConfiguration() {
        try {
            ...
            if (!uiConfigFile.exists()) {
                ...
                this.authEnabled = DEFAULT_AUTH_ENABLED;
            ...
            this.authEnabled = Boolean.parseBoolean(uiProps.getProperty(AUTH_ENABLED_KEY, String.valueOf(DEFAULT_AUTH_ENABLED)));
            this.apiToken = uiProps.getProperty(AUTH_TOKEN_KEY, DEFAULT_API_TOKEN);
