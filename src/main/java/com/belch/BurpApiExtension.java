...
public class BurpApiExtension implements BurpExtension {
    ...
    private void initializeApiServerAndUI() {
        ...
        // Create and register config panel before starting server so port can be changed if bind fails
        ...
        logger.info("[+] Configuration panel registered (Belch Config tab)");
    ...
    private void startApiServer() {
        ...
        try {
            ...
            boolean portInUse = e.getMessage() != null && (e.getMessage().contains("Port already in use") || e.getMessage().contains("Address already in use"))
                || e.getCause() instanceof java.net.BindException;
            if (portInUse) {
                String hint = " On macOS, port 5000 is often used by AirPlay—use 7850 or 8000.";
                String path = System.getProperty("user.home") + "/.belch/extension.properties";
                api.logging().logToError("Belch: Port " + port + " is already in use. Set api.port=7850 in " + path + " and reload the extension." + hint);
                throw new RuntimeException("API server startup failed: port " + port + " is already in use. Edit " + path + " (set api.port=7850 or 8000) and reload." + hint, e);
            }
